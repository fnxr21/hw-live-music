...
-- Function: handle live playlist updates and sync song_request status
...
BEGIN
    ...
    -- Get UUID for PLAYED status
    SELECT status_id INTO played_status
    FROM live_music.ref_song_status
    WHERE status_name = 'PLAYED';
    ...
    ----------------------------------
    IF NEW.order_number = 1 AND OLD.order_number IS DISTINCT FROM NEW.order_number THEN
        -- Mark corresponding song_request as PLAYED
        UPDATE live_music.trx_song_requests
        SET status = played_status,
            updated_at = NOW()
        WHERE song_request_id = NEW.song_request_id;
    END IF;
